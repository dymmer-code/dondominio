defmodule DonDominio.Telemetry do
  @moduledoc """
  Logs each real (non-stubbed) DonDominio HTTP request at `:debug` level,
  replacing the `Tesla.Middleware.Logger` configuration this library used
  before migrating to Req -- Req itself doesn't emit its own
  ...
  def handle_event(
        [:finch, :request, :stop],
        measurements,
        %{name: DonDominio.Finch} = metadata,
        _config
      ) do
  ...
  defp log(%Finch.Request{method: method, path: path}, duration_native, status) do
    ...
    Logger.debug("#{method} /domain#{path} ===> #{status} / time=#{time_ms}ms")
  end
end
